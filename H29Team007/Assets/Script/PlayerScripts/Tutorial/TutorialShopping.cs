...
public class TutorialShopping : MonoBehaviour {
    ...
    private TutorialBasket basketScript;
    ...
    void Start()
    {
        ...
        //2個目
        baggage2Script = transform.Find("SecondBaggage").GetComponent<TutorialAngleManager>();
    ...
    {
        return (myBaggage2.Count + myBaggage.Count);
    }

    public bool IsPlasticBag()
    {
        bool result = false;
        if (myBaggage.Count > 0) result = (myBaggage[0].transform.tag == "Plasticbag");
        return result;
    }
    ...
    public void BaggegeFall(Vector3 startPos)
    {
        for (int i = 0; i < myBaggage.Count; i++)
        {
            ...
            TutrialFallDown fall = myBaggage[i].GetComponent<TutrialFallDown>();
            fall.enabled = true;
    ...
    public void BaggegeFall2(Vector3 startPos)
    {
        for (int i = 0; i < myBaggage2.Count; i++)
        {
            ...
            TutrialFallDown fall = myBaggage2[i].GetComponent<TutrialFallDown>();
            fall.enabled = true;
            fall.SetPoint(pos, sp);
